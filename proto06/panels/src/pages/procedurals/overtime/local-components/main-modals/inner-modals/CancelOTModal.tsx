...
import { OVERTIMEViewAction, OVERTIMEViewAllFilterApproverAction } from '@/store/actions/procedurals';
...
export default function CancelOTModal(props: Props) {
    ...
    const fetchOTByApprover = async() => {
        await axiosInstance.get(`ot/`,{
          params:{
            approver: currUser?.emp_no
          }
        }).then(res => {
          dispatch(OVERTIMEViewAllFilterApproverAction({data: res.data}))
        })
    }
    const cancelApprovedOT = async () => {
        ...
        const payload = {
            ...
            status: "cancel",
            ot_reason_cancelled: `${cancelReason} (${currUser?.emp_no})`,
            added_by: currUser?.emp_no,

        }
        await axiosInstance.put(`ot_new/${data.id}/`, payload).then((res:AxiosResponse) => {
            
            isDepartmentManager? fetchOTByApprover(): dispatch(OVERTIMEViewAction())
            ...
            dispatch(HandleAlertAction({
                open:true,
                status:"success",
                message:"Cancel approved OT successfully"
            }))
      ...
      >
        <Box sx={style}>
            ...
            <Typography id="modal-modal-description" className="text-red-500" sx={{ mt: 2 }}>
                Are you sure you want to <span className="font-bold">CANCEL</span> the approved OT?
            </Typography>
            ...
            <div className='m-auto'>
                ...
                <Button disabled={isLoading} className="" onClick={() => cancelApprovedOT()}>Yes</Button>
            </div>
        </Box>
      </Modal>
    </div>
