...
export default function CancelUAModal(props: Props) {
    ...
    const fetchUAByApprover = async() => {
        await axiosInstance.get(`ua/`,{
          params:{
    ...
    const cancelApprovedUA = async () => {
        ...
        const payload = {
            ...
            status: "cancel",
            ua_reason_cancelled: `${cancelReason} (${currUser?.emp_no})`,
            added_by: currUser?.emp_no,

        }
        await axiosInstance.put(`ua_new/${data.id}/`, payload).then((res:AxiosResponse) => {
            
            isDepartmentManager? fetchUAByApprover(): dispatch(OVERTIMEViewAction())
            ...
            dispatch(HandleAlertAction({
                open:true,
                status:"success",
                message:"Cancel approved UA successfully"
            }))
      ...
      >
        <Box sx={style}>
            ...
            <Typography id="modal-modal-description" className="text-red-500" sx={{ mt: 2 }}>
                Are you sure you want to <span className="font-bold">CANCEL</span> the approved UA?
            </Typography>
            ...
            <div className='m-auto'>
                ...
                <Button disabled={isLoading} className="" onClick={() => cancelApprovedUA()}>Yes</Button>
            </div>
        </Box>
      </Modal>
    </div>
