...
import { APPRAISALDETAILSPageDescriptions, APPRAISALDETAILSPageColumns } from '@/data/pages-data/quick-accesses-data/appraisal-details-data';
...
export default function APPRAISALDETAILSPage() {
  ...
  const { data, status, error } = KPICOREViewSpecificEmployee;
  ...
  useEffect(()=> {
    if(emp_no){
      dispatch(KPICOREViewSpecificEmployeeAction({emp_no: (+emp_no)}))
    }
  }, [emp_no]);
    ...
    <Fragment>
      <div className="my-10 flex flex-wrap justify-between items-start gap-6">
        ...
        <Typography style={{width: "100%", fontSize: "12px", fontWeight: "400", marginTop: '4px'}}>
          <p>Status: {APPRAISALDETAILSPageDescriptions}</p>
        </Typography>
      ...
      <div style={{ height: '660px', width: '100%' }} id="printable-area">
        <DataGrid
          ...
          columns={APPRAISALDETAILSPageColumns}
          initialState={{
            pagination: {
