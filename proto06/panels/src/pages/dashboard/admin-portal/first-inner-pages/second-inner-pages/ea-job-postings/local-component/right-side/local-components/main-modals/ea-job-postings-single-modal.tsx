import { Dispatch, SetStateAction, Fragment, useState } from 'react';
import Modal from '@mui/joy/Modal';
import ModalDialog from '@mui/joy/ModalDialog';
import { Transition } from 'react-transition-group';
import { JOBPOSTINGSViewInterface } from '@/types/types-employee-and-applicants';
import JOBPOSTINGSModalComponent from './inner-modals/job-postings-modal-component';


interface SingleJOBPOSTINGSInterface {
    singleJOBPOSTINGSOpenModal: boolean; 
    setSingleJOBPOSTINGSOpenModal: Dispatch<SetStateAction<boolean>>;
    singleJOBPOSTINGSDetailsData: JOBPOSTINGSViewInterface;
    setSingleJOBPOSTINGSDetailsData: Dispatch<SetStateAction<JOBPOSTINGSViewInterface>>;
}

export default function ViewJOBPOSTINGSSingleModal(props: SingleJOBPOSTINGSInterface) {
    const {singleJOBPOSTINGSOpenModal, setSingleJOBPOSTINGSOpenModal, setSingleJOBPOSTINGSDetailsData, singleJOBPOSTINGSDetailsData} = props;
  const [scroll, setScroll] = useState<boolean>(true);
  return (
    <Fragment>
      <Transition in={singleJOBPOSTINGSOpenModal} timeout={400}>
      {(state: string) => (
      <Modal
        keepMounted
        open={!['exited', 'exiting'].includes(state)}
        onClose={() => {
          setSingleJOBPOSTINGSOpenModal(false);
        }}
        slotProps={{
            backdrop: {
              sx: {
                opacity: 0,
                backdropFilter: 'none',
                transition: `opacity 400ms, backdrop-filter 400ms`,
                ...{
                  entering: { opacity: 1, backdropFilter: 'blur(8px)' },
                  entered: { opacity: 1, backdropFilter: 'blur(8px)' },
                }[state],
              },
            },
          }}
          sx={{
            visibility: state === 'exited' ? 'hidden' : 'visible',
          }}
      >
        <ModalDialog 
            aria-labelledby="dialog-vertical-scroll-title" 
            layout={'center'}
            sx={{
              ...JOBPOSTINGSModalArea,
                opacity: 0,
                transition: `opacity 300ms`,
                ...{
                  entering: { opacity: 1 },
                  entered: { opacity: 1 },
                }[state],
                overflow: 'auto',
            }}
        >
          <JOBPOSTINGSModalComponent setSingleJOBPOSTINGSDetailsData={setSingleJOBPOSTINGSDetailsData} singleJOBPOSTINGSDetailsData={singleJOBPOSTINGSDetailsData} scroll={scroll} setScroll={setScroll}/>
        </ModalDialog>
      </Modal>
        )}
      </Transition>
    </Fragment>
  );
}


// Styles
const JOBPOSTINGSModalArea = {
  height: '180.5mm',
  width: '210mm',
  margin: '0 auto',
  background: 'white',
  boxShadow: '0px 0px 5px rgba(0, 0, 0, 0.1)',
  overflow: 'hidden',
};