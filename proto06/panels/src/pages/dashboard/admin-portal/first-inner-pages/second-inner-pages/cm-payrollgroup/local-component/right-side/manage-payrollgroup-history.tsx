    ...
    <Fragment>
      ...
      <div style={{ height: '600px', width: '100%' }}>
        <DataGrid
          ...
          localeText={{ noRowsLabel: `${status === 'loading' ? `${status?.toUpperCase()}...` : status === 'failed' ?  'No cutoff lists found. Contact your administrator/support.' : (status === null || status === undefined) ? 'The caller for PAYROLLGROUP Epic hasn\'t been set up, please contact your frontend developer': 'There is no PAYROLLGROUP to generate. Double check with a Database Admin'}` }}
        />
      </div>
    </Fragment>
  );
